								...
								<c:forEach var="n" items="${list }">
									<tr>
										...
										<td class="title indent text-align-left"><a
											href="detail?no=${n.no }">${n.title } <span>[${n.cmtCount }]</span>
										</a></td>
										<td>${n.id }</td>
										<td><fmt:formatDate pattern="yyyy-MM-dd"
												value="${n.regdate }" /></td>
										<td>${n.hit }</td>
										<td><input type="checkbox" name="open-no" value="${n.no }"></td>
										<td><input type="checkbox" name="del-no" value="${n.no }"></td>
									</tr>
								</c:forEach>
					...
					<ul class="-list- center">
						<c:forEach begin="0" end="4" var="n">
							<c:if test="${(startNum + n) <= lastNum }">
								<li><a
									class="-text- ${(page==(startNum + n)) ? 'orange' : '' } bold"
									href="?p=${startNum + n }&f=${param.f }&q=${param.q}">${startNum + n }</a></li>
							</c:if>
						</c:forEach>
					</ul>
